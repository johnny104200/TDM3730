...
unsigned char src_mac[6] = {0x00, 0x0c, 0x43, 0xa0, 0x34, 0xc2};
unsigned char pre_mac[6] = {0x00, 0x0c, 0x43, 0xa0, 0x34, 0xf9};
...
int Uplayer_layer_Set_PeerN_Proc( 
	char * 		arg);
int	Uplayer_Show_MacTable_Proc(	
	char *		extra);
int Uplayer_layer_Remove_PeerN_Proc( 
	char * 		arg);
...
void init_dev_desc( DEV_MAIN_DISCRIPTION *dev_desc)
{
...
#if 0	
	dev_desc->sock = socket(AF_PACKET, SOCK_RAW, htons(ETH_P_ALL));
	if (dev_desc->sock == -1) { 
	...
	close(fd);
	memcpy( dev_desc->src_mac, ifr.ifr_hwaddr.sa_data,ETH_MAC_LEN );
	printf("%s mac address:%2x:%2x:%2x:%2x:%2x:%2x\n",ETH_NAME , dev_desc->src_mac[0],dev_desc->src_mac[1],dev_desc->src_mac[2],dev_desc->src_mac[3],dev_desc->src_mac[4],dev_desc->src_mac[5]);
#elif 0
	struct net_device *pNetDev = NULL;
	struct net_device *dev;

	pNetDev = alloc_etherdev(0);
	pNetDev->netdev_ops  = (void*)vmalloc(sizeof(struct net_device_ops));
	if (pNetDev->netdev_ops) {
		memset(pNetDev->netdev_ops,0, sizeof(struct net_device_ops)); 
		return ;
	}
	dev_get_by_name(pNetDev->nd_net, pDevName);
	dev = dev_get_by_name(pNetDev, "ra0");
    dev->dev_addr; // is the MAC address
#else  // try to get wireless device mac address
	int retval = SetMacAddress( dev_desc->src_mac, mymac);
	if (retval)
		printk ("My MacAddress = %02x:%02x:%02x:%02x:%02x:%02x\n",PRINT_MAC(dev_desc->src_mac));
	retval = SetMacAddress( next_mac, nextmac);
	if (retval)
		printk ("Next MacAddress = %02x:%02x:%02x:%02x:%02x:%02x\n",PRINT_MAC(next_mac));
	else
		memset(next_mac,0,6);
	retval = SetMacAddress( pre_mac, premac);
	if (retval)
		printk ("Pre MacAddress = %02x:%02x:%02x:%02x:%02x:%02x\n",PRINT_MAC(pre_mac));	
	else
		memset(pre_mac,0,6);
	/*prepare sockaddr_ll*/
	/*RAW communication*/
...
void uart_rx(void)
{
	...
	while(uart_number<USRT_BUFF_SIZE) {
		ret = serail_get_char_directly(&cTmp, 0);
		if(ret>0) 
		{
...
static void ksocket_start(void)
{
	...
	init_uart_send_buf();
	//set_serial_func_ptr(uart_rx_char);
#endif	
	...
	for(;;) {	
        ...
        if(size==ETH_FRAME_LEN || size%300==0) {
			printk("gState->dst_mac=%02x ",gState->socket_address.sll_addr[5]);
			if(gState->socket_address.sll_addr[5] == pre_mac[5])
				hex_dump("Receive Packet", buf, size);
			printk("state=%d sub_state=%d\n",gState->dev_state,gState->sub_state);
